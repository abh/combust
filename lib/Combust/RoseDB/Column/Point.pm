...
use Rose::DB::Object::Constants qw(PRIVATE_PREFIX FLAG_DB_IS_PRIVATE STATE_IN_DB STATE_LOADING
    ...
    if ($interface eq 'get_set') {
            ...
            $methods{$name} = sub {
                ...
                if ($self->{STATE_SAVING()}) {
                    $self->{$formatted_key, $driver} =
                      $column_obj->format_array($db, $self->{$key})
                ...
                if (defined $self->{$formatted_key, $driver}) {
                    $self->{$key} =
                      $column_obj->parse_array($db, $self->{$formatted_key, $driver});
                    $self->{$formatted_key, $driver} = undef;
        ...
        if (defined $default) {
            $methods{$name} = sub {
                ...
                if (!defined $self->{$key}
                    && (!$self->{STATE_SAVING()} || !defined $self->{$formatted_key, $driver}))
                {
                    $self->{$key} = $column_obj->parse_array($db, $default);
