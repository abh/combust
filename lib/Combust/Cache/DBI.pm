...
sub fetch {
    ...
    $row->{data} = Encode::decode("utf8", Compress::Zlib::memGunzip($row->{data}))
...
sub store {
    ...
    if ($HAVE_ZLIB && $len >= COMPRESS_THRESHOLD) {
        my $c_val = Compress::Zlib::memGzip(Encode::encode("utf8", $data));
