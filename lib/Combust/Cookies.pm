...
sub bake_cookies {
    ...
    for my $cookie_name (keys %special_cookies, $default_cookie_name) {
        ...
        my @keys =
          $special_cookies{$cookie_name} ? @{$special_cookies{$cookie_name}} : (keys %$cookies);
...
sub check_cookie {
    ...
    if ($cookie_version eq '2') {
        ...
        ($cookie_version, my $ts, $cookie, my $hex_cs) =
          $raw_id =~ m!^(\d+)/(\d+)/(.*?)/([^/]{8})$!;
