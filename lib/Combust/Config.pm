...
sub _setup_dbs {
    ...
    my %sections =
      map { $_ =~ s/^database-(.*?)\..*/$1/; $_ => 1 } grep {m/^database/} keys %Config;
    my $got_default = 0;
...
sub apache_modules {
    ...
    my $modules = $cfg->param('apache.modules')
...
sub static_headers {
    ...
    my $sitename = shift or carp "sitename parameter required" and return;
    my %headers =
      (%{$cfg->get_block("headers-global")}, %{$cfg->get_block("headers-${sitename}")});
    return %headers;
}
